...
const bookaService = async (req, res) => {
  ...
  let shopid = null,
    date = new Date();
  await serviceSchema
    .findById({ _id: req.params.serviceid })
    .exec()
    .then((datas) => {
      shopid = datas.shopid;
    })
...
const viewBookingByWid = (req, res) => {
  serviceBookings
    .find({ shopid: req.params.id, approvalstatus: false })
    .populate("custid")
    .populate("serviceid")
    .exec()
    .then((data) => {
...
const updatePaymentbyBookingId = (req, res) => {
  serviceBookings
    .findByIdAndUpdate({ _id: req.params.id }, { paymentstatus: true })
...
const viewBookingByMechid = (req, res) => {
  serviceBookings
    .find({ mechid: req.params.id, completedStatus: false })
    .populate("custid")
    .populate("serviceid")
    .exec()
    .then((data) => {
...
const closeBookingById = (req, res) => {
  serviceBookings
    .findByIdAndUpdate({ _id: req.params.id }, { completedStatus: true })
