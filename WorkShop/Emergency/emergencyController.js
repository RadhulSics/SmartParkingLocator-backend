...
const viewEmergencyForShopid = async (req, res) => {
  try {
      // Fetch active workshops' districts
      const workshop = await workshopSchema.findById(req.params.id)
...
const viewEmergencyByMechid = (req, res) => {
  emergencySchema.find({mechid:req.params.id,completedStatus:false})
  .populate('custid')
  .exec()
    .then(data => {
      // console.log(data);
